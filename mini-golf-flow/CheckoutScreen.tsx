 ...
 * total, not the generic "Complete booking" this used to say) completes
 * payment and calls `onComplete` directly, advancing straight to the
 * Confirmation screen (node 4281:91224) — the real Terms & Conditions
 * overlay (node 4281:91061) belongs to that next screen's own "Complete
 * registration" action, not this one. Per node 4281:91137, the button lives
 * in a sticky bar pinned to the bottom of the screen (not simply after the
 * last scrolled section), and stays disabled until the guest has a verified
 * phone number on file — a signed-in guest's counts as already verified; an
 * unsigned guest has to complete the phone OTP flow in Contact Information
 * first.
 */
  ...
  return (
    <div className="pk-proto-screen">
      ...
      <div className="pk-proto-screen__sticky-cta">
        <Button variant="primary" disabled={!phoneVerified} onClick={onComplete}>
